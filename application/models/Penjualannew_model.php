...
class Penjualannew_model extends CI_Model {
	private $table;
	private $select_default;
	function __construct(){
        parent::__construct();
		$this->table = "sales_data";
		$this->select_default = '*, sales_data.sales_id as id, sales_data.date as date';
	}
	
	public function get_all($limit_offset = array()){
		$this->db->select($this->select_default);
		$this->db->join('sales_transaction', 'sales_data.sales_id = sales_transaction.id');
		$this->db->join('product', 'product.id = sales_data.product_id');
		$this->db->order_by("sales_data.date", "desc");
	...
	public function get_filter($filter = '',$limit_offset = array(),$is_array = false){
		...
		// $this->db->join('sales_data', 'sales_data.sales_id = sales_transaction.id');
		$this->db->join('sales_transaction', 'sales_data.sales_id = sales_transaction.id');
		$this->db->join('product', 'product.id = sales_data.product_id');
		$this->db->order_by("sales_data.date", "desc");
		if(!empty($filter)){
  ...
  public function get_filter_c($filter = '',$limit_offset = array(),$is_array = false){
		$this->db->select($this->select_default);
		// $this->db->join('customer', 'customer.id = sales_transaction.customer_id', 'left');
		// $this->db->join('sales_data', 'sales_data.sales_id = sales_transaction.id');
		$this->db->join('sales_transaction', 'sales_data.sales_id = sales_transaction.id');
